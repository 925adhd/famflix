...
export default async function Home() {
  ...
  const { data: titles } = await supabase
    .from("titles")
    .select(
      "id, name, year, kind, overview, poster_url, backdrop_url, tmdb_id, created_at, duration_seconds"
    )
    .eq("status", "ready")
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <section
        className={`${continueWatching.length > 0 ? "" : "-mt-16 sm:-mt-20"} px-6 pb-16 sm:px-12 lg:px-20`}
      >
