...
export function UploadForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-2.5 sm:gap-4">
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <textarea
          ...
          rows={2}
          disabled={busy}
          className="rounded border border-white/10 bg-white/5 px-3 py-2 outline-none focus:border-white/30 disabled:opacity-50 sm:[&]:min-h-[5.5rem]"
        />
      </label>

      <label className="flex flex-col gap-1 text-sm">
        <span className="text-zinc-400">Video file</span>
        <input
          ref={fileInputRef}
          type="file"
          accept="video/*"
      ...
      <button
        type="submit"
        disabled={busy}
        className="rounded bg-accent py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 sm:mt-2"
      >
